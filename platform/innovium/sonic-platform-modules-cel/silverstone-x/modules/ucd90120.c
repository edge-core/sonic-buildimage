...
#define UCD9000_MONITOR_CONFIG_LEN		13 // 13 bytes(length + 12 configurations) long to meet ucd90120
...
static int ucd9000_get_mfr_status(struct i2c_client *client, u8 *buffer)
{
	...
	return i2c_smbus_read_i2c_block_data(client, UCD9000_MFR_STATUS, UCD9000_MFR_STATUS_LEN, buffer);
}
	...
	if (mid->driver_data == ucd90124) {
		...
		for (i = 0; i < UCD9000_NUM_FAN; i++) {
			...
			ret = i2c_smbus_read_i2c_block_data(client,
							UCD9000_FAN_CONFIG, UCD9000_FAN_CONFIG_LEN,
							data->fan_data[i]);
			if (ret < 0)
				return ret;
		}

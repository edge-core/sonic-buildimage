...
#include "x86-64-cameo-esc600-128q-power.h"
...
ssize_t psu_status_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    switch (attr->index)
    {
        case 1:
            ...
            break;
        case 2:
            if(status & BIT_5_MASK)
            {
                result = FALSE;
            }
            break;
        case 3:
            if(status & BIT_6_MASK)
            {
                result = FALSE;
            }
            break;
        case 4:
            if(status & BIT_7_MASK)
            {
                result = FALSE;
            }
            break;
    ...
    if( bmc_enable() == ENABLE)
    {
        switch(attr->index)
        {
            ...
            case PSU2_VIN:
                ...
                break;
            case PSU3_VIN:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_3_VIN_REG);
                break;
            case PSU4_VIN:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_4_VIN_REG);
                break;
        }
    ...
    if( bmc_enable() == ENABLE)
    {
        switch(attr->index)
        {
            ...
            case PSU2_IIN:
                ...
                break;
            case PSU3_IIN:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_3_IIN_REG);
                break;
            case PSU4_IIN:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_4_IIN_REG);
                break;
        }
        if(u16_val == 0xffff || u16_val == -1)
        {
    ...
    if( bmc_enable() == ENABLE)
    {
        switch(attr->index)
        {
            ...
            case PSU2_IOUT:
                ...
                break;
            case PSU3_IOUT:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_3_IOUT_REG);
                break;
            case PSU4_IOUT:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_4_IOUT_REG);
                break;
        }
        if(u16_val == 0xffff || u16_val == -1)
        {
    ...
    if( bmc_enable() == ENABLE)
    {
        switch(attr->index)
        {
            ...
            case PSU2_TEMP:
                ...
                break;
            case PSU3_TEMP:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_3_TEMP_1_REG);
                break;
            case PSU4_TEMP:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_4_TEMP_1_REG);
                break;
        }
        if(u16_val == 0xffff || u16_val == -1)
        {
    ...
    if( bmc_enable() == ENABLE)
    {
        switch(attr->index)
        {
            ...
            case PSU2_FAN_SPEED:
                ...
                break;
            case PSU3_FAN_SPEED:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_3_FAN_SPEED_REG);
                break;
            case PSU4_FAN_SPEED:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_4_FAN_SPEED_REG);
                break;
        }
        if(u16_val == 0xffff || u16_val == -1)
        {
    ...
    if( bmc_enable() == ENABLE)
    {
        switch(attr->index)
        {
            ...
            case PSU2_POUT:
                ...
                break;
            case PSU3_POUT:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_3_POUT_REG);
                break;
            case PSU4_POUT:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_4_POUT_REG);
                break;
        }
        if(u16_val == 0xffff || u16_val == -1)
        {
    ...
    if( bmc_enable() == ENABLE)
    {
        switch(attr->index)
        {
            ...
            case PSU2_PIN:
                ...
                break;
            case PSU3_PIN:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_3_PIN_REG);
                break;
            case PSU4_PIN:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_4_PIN_REG);
                break;
        }
        if(u16_val == 0xffff || u16_val == -1)
        {
    ...
    if( bmc_enable() == ENABLE)
    {
        switch(attr->index)
        {
            ...
            case PSU2_MFR_IOUT_MAX:
                ...
                break;
            case PSU3_MFR_IOUT_MAX:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_3_MFR_IOUT_MAX_REG);
                break;
            case PSU4_MFR_IOUT_MAX:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, PSU_4_MFR_IOUT_MAX_REG);
                break;
        }
        if(u16_val == 0xffff || u16_val == -1)
        {
...
ssize_t dc_vout_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if( bmc_enable() == ENABLE)
    {
        ...
        {
            case DC_6E_P0_VOUT:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, DC_6E_P0_VOUT_REG);
                break;
            case DC_70_P0_VOUT:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, DC_70_P0_VOUT_REG);
                break;
            case DC_70_P1_VOUT:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, DC_70_P1_VOUT_REG);
                break;
        }
        if(u16_val == 0xffff || u16_val == -1)
        {
...
ssize_t dc_iout_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if( bmc_enable() == ENABLE)
    {
        ...
        {
            case DC_6E_P0_IOUT:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, DC_6E_P0_IOUT_REG);
                break;
            case DC_70_P0_IOUT:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, DC_70_P0_IOUT_REG);
                break;
            case DC_70_P1_IOUT:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, DC_70_P1_IOUT_REG);
                break;
        }
        if(u16_val == 0xffff || u16_val == -1)
        {
...
ssize_t dc_pout_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if( bmc_enable() == ENABLE)
    {
        ...
        {
            case DC_6E_P0_POUT:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, DC_6E_P0_POUT_REG);
                break;
            case DC_70_P0_POUT:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, DC_70_P0_POUT_REG);
                break;
            case DC_70_P1_POUT:
                u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, DC_70_P1_POUT_REG);
                break;
        }
        if(u16_val == 0xffff || u16_val == -1)
        {
...
ssize_t dc_11_p0_vout_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if( bmc_enable() == ENABLE)
    {
        u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, dc_11_vout_table[attr->index][0]);
        if(u16_val == 0xffff || u16_val == -1)
        {
            return sprintf(buf, "%s0\n", buf);
        }
...
ssize_t dc_11_p1_vout_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if( bmc_enable() == ENABLE)
    {
        u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, dc_11_vout_table[attr->index][1]);
        if(u16_val == 0xffff || u16_val == -1)
        {
            return sprintf(buf, "%s0\n", buf);
        }
...
ssize_t dc_12_p0_vout_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if( bmc_enable() == ENABLE)
    {
        u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, dc_12_vout_table[attr->index][0]);
        if(u16_val == 0xffff || u16_val == -1)
        {
            return sprintf(buf, "%s0\n", buf);
        }
...
ssize_t dc_12_p1_vout_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if( bmc_enable() == ENABLE)
    {
        u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, dc_12_vout_table[attr->index][1]);
        if(u16_val == 0xffff || u16_val == -1)
        {
            return sprintf(buf, "%s0\n", buf);
        }
...
ssize_t dc_13_p0_vout_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if( bmc_enable() == ENABLE)
    {
        u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, dc_13_vout_table[attr->index][0]);
        if(u16_val == 0xffff || u16_val == -1)
        {
            return sprintf(buf, "%s0\n", buf);
        }
...
ssize_t dc_13_p1_vout_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if( bmc_enable() == ENABLE)
    {
        u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, dc_13_vout_table[attr->index][1]);
        if(u16_val == 0xffff || u16_val == -1)
        {
            return sprintf(buf, "%s0\n", buf);
        }
...
ssize_t dc_11_p0_iout_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if( bmc_enable() == ENABLE)
    {
        u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, dc_11_iout_table[attr->index][0]);
        if(u16_val == 0xffff || u16_val == -1)
        {
            return sprintf(buf, "%s0\n", buf);
        }
...
ssize_t dc_11_p1_iout_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if( bmc_enable() == ENABLE)
    {
        u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, dc_11_iout_table[attr->index][1]);
        if(u16_val == 0xffff || u16_val == -1)
        {
            return sprintf(buf, "%s0\n", buf);
        }
...
ssize_t dc_12_p0_iout_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if( bmc_enable() == ENABLE)
    {
        u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, dc_12_iout_table[attr->index][0]);
        if(u16_val == 0xffff || u16_val == -1)
        {
            return sprintf(buf, "%s0\n", buf);
        }
...
ssize_t dc_12_p1_iout_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if( bmc_enable() == ENABLE)
    {
        u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, dc_12_iout_table[attr->index][1]);
        if(u16_val == 0xffff || u16_val == -1)
        {
            return sprintf(buf, "%s0\n", buf);
        }
...
ssize_t dc_13_p0_iout_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if( bmc_enable() == ENABLE)
    {
        u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, dc_13_iout_table[attr->index][0]);
        if(u16_val == 0xffff || u16_val == -1)
        {
            return sprintf(buf, "%s0\n", buf);
        }
...
ssize_t dc_13_p1_iout_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if( bmc_enable() == ENABLE)
    {
        u16_val = i2c_smbus_read_word_data(Cameo_BMC_14_client, dc_13_iout_table[attr->index][1]);
        if(u16_val == 0xffff || u16_val == -1)
        {
            return sprintf(buf, "%s0\n", buf);
        }

...
static int mcp3422_read(struct mcp3422 *adc, int *value, u8 *config)
{
	...
	if (sample_rate == MCP3422_SRATE_3) {
		//ret = i2c_master_recv(adc->i2c, buf, 4);
                 ret = i2c_smbus_read_i2c_block_data(adc->i2c, 0, 4 , buf);
		temp = buf[0] << 16 | buf[1] << 8 | buf[2];
		*config = buf[3];
	} else {
		//ret = i2c_master_recv(adc->i2c, buf, 3);
                ret = i2c_smbus_read_i2c_block_data(adc->i2c, 0, 3 , buf);
		temp = buf[0] << 8 | buf[1];
		*config = buf[2];
	}
...
MODULE_DESCRIPTION("Modify from mcp3422.c(Microchip mcp3421/2/3/4/5/6/7/8 driver)");

...
class Sfp(SfpBase):
    ...
    def __init__(self, index, eeprom_path_list, sfp_type, ext_sysfile_list=None):
        ...
        if self.sfp_type == QSFP_TYPE:
            self.present_file = ext_sysfile_list[0]+ 'qsfp{}_present'.format(self.index)
            self.reset_file = ext_sysfile_list[0]+ 'qsfp{}_reset'.format(self.index)
            self.lp_file = ext_sysfile_list[0]+ 'qsfp{}_low_power'.format(self.index)
        else:

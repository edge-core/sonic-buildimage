...
static int Cameo_i2c_probe(struct i2c_client *client, const struct i2c_device_id *dev_id)
{
    ...
    status = sysfs_create_group(&client->dev.kobj, &ESQC610_THERMAL_group);
    if (status)
    {
        goto exit_free;
    }
    
    status = sysfs_create_group(&client->dev.kobj, &ESQC610_POWER_group);
    if (status)
    {
        goto exit_free;
    }
...
exit_remove:
    ...
    sysfs_remove_group(&client->dev.kobj, &ESQC610_THERMAL_group);
    sysfs_remove_group(&client->dev.kobj, &ESQC610_POWER_group);

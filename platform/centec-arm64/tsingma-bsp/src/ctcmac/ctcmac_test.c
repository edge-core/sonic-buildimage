/*
 * Centec cpu_mac Ethernet Driver For Test -- cpu_mac controller implementation
...
static int cpumac_unit_irq_installed;
static struct regmap *regmap_base;
struct cpu_mac_unit_regs *g_cpumacu_reg;
				     ...
				     struct ethtool_eeprom *ee, u8 *data)
{
	...
	if (ee->offset >= 0x00030000) {
		memset(&pkt_stats[priv->index], 0,
		       sizeof(struct ctcmac_pkt_stats));
		return 0;
	} else if (ee->offset >= 0x00020000) {
		val =
		    data[0] | (data[1] << 8) | (data[2] << 16) | (data[3] <<
								  24);
		ctcmac_fill_test_param(priv->index, ee->offset - 0x00020000,
				       val);
		return 0;
...
static void ctcmac_ethtool_get_stats(struct net_device *netdev,
...
static int startup_ctcmac(struct net_device *ndev)
{
	...
	for (i = 0; i < priv->num_tx_queues; i++) {
		priv->tx_queue[i]->tx_ring_size =
		    test_param[priv->index].ring_size;
...
static void ctcmac_process_frame(struct net_device *ndev, struct sk_buff *skb)
{
	...
	if (test_param[priv->index].rxts_en) {
		...
		if (test_param[priv->index].rxts_dump)
			pr_err("receive frame time stamp 0x%llx\n", *ns);

		memset(shhwtstamps, 0, sizeof(*shhwtstamps));
		shhwtstamps->hwtstamp = ns_to_ktime(be64_to_cpu(*ns));
		skb_pull(skb, 8);
		if (test_param[priv->index].rxts_dump) {
			pr_err("receive frame time stamp %lld\n",
...
static int ctc_mac_serdes_init(struct ctcmac_private *priv)
{
	...
	writel(0x0026c03a, &priv->cpumacu_reg->cpu_mac_unit_hss_cfg[6]);
	writel(0x28061810, &priv->cpumacu_reg->cpu_mac_unit_hss_cfg[8]);
	writel(0x0026c03a, &priv->cpumacu_reg->cpu_mac_unit_hss_cfg[12]);
...
static void ctcmac_hw_init(struct ctcmac_private *priv)
{
	...
	pr_err("%s ctcmac_hw_init 0x%x\n", __func__, priv->ndev->flags);
}
...
{
	struct cpu_mac_unit_mems *mems =
	    (struct cpu_mac_unit_mems *)(g_cpumacu_reg + 0x400 -
				       CPUMACUNIT_REG_BASE);

	ctcmac_regw(&g_cpumacu_reg->cpu_mac_unit_interrupt_func[2], 1);
	ctcmac_regw(&g_cpumacu_reg->cpu_mac_unit_interrupt_func[1], 1);

	while (ctcmac_regr(&g_cpumacu_reg->cpu_mac_unit_fifo_status)) {
		pr_err("Tx Capture time stamp in fifo 0x%x 0x%x 0x%x\n",
		       ctcmac_regr(&mems->cpu_mac_unit_tx_capture_ts_0[0]),
		       ctcmac_regr(&mems->cpu_mac_unit_tx_capture_ts_0[1]),
		       ctcmac_regr(&mems->cpu_mac_unit_tx_capture_ts_0[2]));
	}
	ctcmac_regw(&g_cpumacu_reg->cpu_mac_unit_interrupt_func[3], 1);
...
static int ctcmac_request_irq(struct ctcmac_private *priv)
{
	...
	if (priv->irqinfo[CTCMAC_UNIT].irq && !cpumac_unit_irq_installed) {
		err =
		    request_irq(priv->irqinfo[CTCMAC_UNIT].irq, ctcmac_unit, 0,
				"cpumac_unit", NULL);
		if (err < 0) {
...
static int ctcmac_clean_rx_ring(struct ctcmac_priv_rx_q *rx_queue,
				int rx_work_limit)
{
	struct net_device *ndev = rx_queue->ndev;
	struct ctcmac_private *priv = netdev_priv(ndev);
					    ...
					    struct sk_buff *skb)
{
	...
	if (!test_param[priv->index].txts_en && !nr_frags &&
		...
		if (test_param[priv->index].txts_en) {
			memcpy(tx_buff->vaddr + offset,
			       &test_param[priv->index].ptp_info, 16);
		}
...
static int ctcmac_probe(struct platform_device *ofdev)
{
	...
	sprintf(priv->irqinfo[CTCMAC_UNIT].name, "%s%s", dev->name, "_unit");

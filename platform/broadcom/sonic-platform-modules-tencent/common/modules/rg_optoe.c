			...
			const struct i2c_device_id *id)
{
	...
	if ((strcmp(client->name, "rg_optoe1") == 0) ||
	    (strcmp(client->name, "rg_sff8436") == 0)) {
		/* one-address (eg QSFP) family */
		optoe->dev_class = ONE_ADDR;
		chip.byte_len = ONE_ADDR_EEPROM_SIZE;
		num_addresses = 1;
	} else if ((strcmp(client->name, "rg_optoe2") == 0) ||
		   (strcmp(client->name, "rg_24c04") == 0)) {
		/* SFP family */
		optoe->dev_class = TWO_ADDR;
		chip.byte_len = TWO_ADDR_EEPROM_SIZE;
		num_addresses = 2;
	} else if (strcmp(client->name, "rg_optoe3") == 0) {
		/* CMIS spec */

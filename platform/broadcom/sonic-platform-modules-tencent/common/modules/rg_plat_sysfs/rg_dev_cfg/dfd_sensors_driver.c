            ...
            uint8_t temp_index, uint8_t temp_attr, char *buf)
{
    ...
    rv = dfd_get_sensor_info(main_dev_id, dev_index, RG_MINOR_DEV_TEMP, temp_index, temp_attr, buf);
    if (rv < 0) {
...
ssize_t dfd_get_psu_sensor_info(uint8_t psu_index, uint8_t sensor_type, char *buf)
{
    int rv, key;
    info_hwmon_buf_f pfunc;
    ...
    DFD_SENSOR_DEBUG(DBG_VERBOSE, "get psu sensor info. psu index:%d, sensor type:%d, key:0x%x,\n",
        psu_index, sensor_type, key);
    pfunc = dfd_deal_hwmon_buf;
    memset(buf, 0 , PAGE_SIZE);
    rv = dfd_info_get_sensor(key, buf, PAGE_SIZE, pfunc);
    if (rv < 0) {
        ...
        DFD_SENSOR_DEBUG(DBG_VERBOSE, "get psu sensor info ok.value:%s\n", buf);
    }
    return rv;
}

...
extern int g_dfd_syseeprom_dbg_level;
...
extern char *g_status_mem_str[STATUS_MEM_END];
...
#define DBG_SYSE2_DEBUG(level, fmt, arg...) do { \
    if (g_dfd_syseeprom_dbg_level & level) { \
        if(level >= DBG_ERROR) { \
            printk(KERN_ERR "[DBG-%d]:<%s, %d>:"fmt, level, __FUNCTION__, __LINE__, ##arg); \
        } else { \
            printk(KERN_INFO "[DBG-%d]:<%s, %d>:"fmt, level, __FUNCTION__, __LINE__, ##arg); \

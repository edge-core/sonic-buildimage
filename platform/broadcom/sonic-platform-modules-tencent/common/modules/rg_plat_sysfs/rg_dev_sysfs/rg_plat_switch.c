 ...
 *
 * This module create a kset in sysfs called /sys/rg_plat
...
struct switch_obj *rg_plat_kobject_create(const char *name, struct kobject *parent)
{
    ...
    if (!obj) {
        SWITCH_DBG("rg_plat_kobject_create %s kzalloc error", name);
        return NULL;
   }
...
void rg_plat_kobject_delete(struct switch_obj **obj)
{
    if (*obj) {
...
static int __init switch_init(void)
{
    ...
    switch_kset = kset_create_and_add("rg_plat", NULL, NULL);
    if (!switch_kset) {

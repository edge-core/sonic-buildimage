...
#define RUJIE_COMMON_DEBUG_VERBOSE(fmt, args...) do {                                        \
    if (g_common_debug_verbose) { \
        printk(KERN_ERR "[RUIJIE_COMMON][VER][func:%s line:%d]\r\n"fmt, __func__, __LINE__, ## args); \
    } \
} while (0)

#define RUJIE_COMMON_DEBUG_ERROR(fmt, args...) do {                                        \
    if (g_common_debug_error) { \
        printk(KERN_ERR "[RUIJIE_COMMON][ERROR][func:%s line:%d]\r\n"fmt, __func__, __LINE__, ## args); \
    } \
} while (0)
...
static int __init ruijie_common_init(void)
{
    int ret;
...
static void __exit ruijie_common_exit(void)
{
    RUJIE_COMMON_DEBUG_VERBOSE("Exit.\n");
}

module_init(ruijie_common_init);
module_exit(ruijie_common_exit);

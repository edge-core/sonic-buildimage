...
class checkBase(object):
    ...
    def checkStatus(self):
        ...
        pmon_debug("%s: current status %s" % (dev_name, curStatus))
        pmon_debug("%s: pre status %s" % (dev_name, self._preDevStatus))
        pmon_debug("%s: peroid_syslog %s" % (dev_name, self._peroid_syslog))
        ...
        if curStatus == STATUS_FAILED:
            ...
            if self._peroid_failed_syslog is not None:
                if getnochangedMsgFlag and time.time() - self._peroid_failed_syslog >= nochangedMsgTime:
                    ...
                    dev_syslog(MSG_UNKNOWN % (dev_name, log))
        ...
        if self._preDevStatus is None:
            ...
            if noprintFirstTimeFlag == 1:
                ...
                if curStatus == STATUS_PRESENT:
                    # present
                    dev_syslog(MSG_IN % dev_name)
                elif curStatus == STATUS_OK:
                    # ok
                    dev_syslog(MSG_OK % dev_name)
                elif curStatus == STATUS_NOT_OK:
                    # not ok
                    dev_syslog(MSG_NOT_OK % dev_name)
                    self._peroid_syslog = time.time()
                else:
                    # absent
                    dev_syslog(MSG_ABSENT % dev_name)
                    self._peroid_syslog = time.time()
        else:
            # from 2nd time...
            if self._preDevStatus == curStatus:
                ...
                if self._preDevStatus == STATUS_ABSENT:
                    ...
                    if curStatus == STATUS_OK:
                        # not ok -> ok
                        dev_syslog(MSG_RECOVER % dev_name)
                        dev_syslog(MSG_OK % dev_name)
                    else:
                        # not ok -> absent
                        dev_syslog(MSG_OUT % dev_name)
...
class platformSyslog():
    def __init__(self):
        self.__sfp_checklist = list()
        self.__fan_checklist = list()
        self.__psu_checklist = list()
        self.__slot_checklist = list()
        self.__temp_checklist = list()
        ...
        if tmpconfig is not None:
            ...
            for location in path:
                ...
                for i in range(len(loc_split_list)):
                    if '*' in loc_split_list[i]:
                        dev_name_index = i
                        break
                locations = glob.glob(location)
                for dev_path in locations:
                    dev_name_list = dev_path.split('/')
                    dev_name = dev_name_list[dev_name_index]     # explame:get eth1 from /sys_switch/transceiver/eth1/present
                    dev_name_alias = tmpconfig.get("alias", {})
                    dev_name = dev_name_alias.get(dev_name, dev_name)
                    dev = checkSfp(dev_path, dev_name, tmpconfig)
        ...
        if tmpconfig is not None:
            ...
            for location in path:
                ...
                for dev_path in locations:
                    ...
                    dev_name_alias = tmpconfig.get("alias", {})
                    dev_name = dev_name_alias.get(dev_name, dev_name)
                    dev = checkFAN(dev_path, dev_name, tmpconfig)
        ...
        if tmpconfig is not None:
            ...
            for location in path:
                ...
                for dev_path in locations:
                    ...
                    dev_name_alias = tmpconfig.get("alias", {})
                    dev_name = dev_name_alias.get(dev_name, dev_name)
                    dev = checkPSU(dev_path, dev_name, tmpconfig)
        ...
        if tmpconfig is not None:
            ...
            for location in path:
                ...
                for dev_path in locations:
                    ...
                    dev_name_alias = tmpconfig.get("alias", {})
                    dev_name = dev_name_alias.get(dev_name, dev_name)
                    dev = checkSlot(dev_path, dev_name, tmpconfig)
        ...
        try:
            ...
            if float(input_temp) >= float(warning_temp):
                if float(input_temp) >= float(critical_temp):
                    if time.time() - temp_item['time'] >= self.__temps_pollingseconds or temp_item['status'] != self.critical_status:
                    ...
                    if time.time() - temp_item['time'] >= self.__temps_pollingseconds or temp_item['status'] != self.warning_status:
                ...
                pmon_debug("%s temperature %sC is in range [%s, %s]" % (temp_name, input_temp, warning_temp, critical_temp))
                temp_item['status'] = self.normal_status
                temp_item['time'] = time.time()
        except Exception as e:
    ...
    def doWork(self):
        try:
            ...
            MSG_EXCEPTION = '%%PMON-5-NOTICE: Exception happened! info:{%s}'

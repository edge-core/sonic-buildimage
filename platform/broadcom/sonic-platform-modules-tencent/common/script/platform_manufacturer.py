...
def exphyfwsplit():
    ...
    if status or len(output) == 0:
        raise Exception("run cmd: {} error, status: {}, msg: {}".format(cmd, status, output))
...
def checksignaldriver(name):
    ...
    status, output = exec_os_cmd(modisexistcmd)
    if status:
        return False
...
def add_5387_driver():
    errmsg = ""
    spi_gpio = "rg_spi_gpio"
    ret = adddriver(spi_gpio)
    if ret is False:
        errmsg = "modprobe rg_spi_gpio driver failed."
        return False, errmsg
    spi_5387_device = "rg_spi_93xx46 spi_bus_num=0"
    ret = adddriver(spi_5387_device)
    if ret is False:
        errmsg = "modprobe rg_spi_93xx46 driver failed."
        return ret, errmsg
    return True, ""


def remove_5387_driver():
    spi_5387_device = "rg_spi_93xx46"
...
def devfileread(path, offset, len, bit_width):
    ret = ""
    val_str = ''
    val_list = []
    fd = -1
    ...
    try:
        ...
        for i in range(0, len, bit_width):
            val_str += " 0x"
    ...
    def get_version(self):
        ...
        try:
            if self.cmd is not None:
                status, output = exec_os_cmd(self.cmd, self.timeout)
                if status or len(output) == 0:
                    raise RuntimeError("run cmd: {} error, status: {}, msg: {}".format(self.cmd, status, output))

...
static int dfd_info_get_sensor_value(int key, uint8_t *buf, int buf_len, info_hwmon_buf_f pfun)
{
    ...
    info_ctrl = dfd_ko_cfg_get_item(key);
    if (info_ctrl == NULL) {
        DBG_DEBUG(DBG_VERBOSE, "can't find dfd config, key: 0x%08x\n", key);
...
int dfd_info_get_sensor(uint32_t key, char *buf, int buf_len, info_hwmon_buf_f pfun)
{
    ...
    if (!DFD_CFG_ITEM_IS_INFO_CTRL(DFD_CFG_ITEM_ID(key)) ||
        (buf == NULL) || buf_len <= 0) {
        DBG_DEBUG(DBG_ERROR, "input arguments error, key: 0x%08x, buf_len: %d\n",
            key, buf_len);
    ...
    if (key_info_ctrl->mode == INFO_CTRL_MODE_CONS) {
        ...
        DBG_DEBUG(DBG_VERBOSE, "get sensor value through int config, key: 0x%08x, value: %d\n", key, key_info_ctrl->int_cons);
        return strlen(buf);
    }

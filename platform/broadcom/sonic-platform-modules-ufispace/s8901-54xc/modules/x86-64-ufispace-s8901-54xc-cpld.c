...
static const struct i2c_device_id cpld_id[] = {
    { "s8901_54xc_cpld1",  cpld1 },
    { "s8901_54xc_cpld2",  cpld2 },
    {}
};
...
static int s8901_54xc_cpld_write_internal(struct i2c_client *client, u8 reg, u8 value)
{
...
/*
int s8901_54xc_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EIO;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = s8901_54xc_cpld_write_internal(cpld_node->client, reg, value);
            break;
        } else {
...
EXPORT_SYMBOL(s8901_54xc_cpld_write);
*/

int s8901_54xc_cpld_psu_mux_sel(u8 mux_sel)
{
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ...
            reg_val = s8901_54xc_cpld_read_internal(cpld_node->client, reg);
            //clear psu_mux_sel bits (bit 1 and 2)
            reg_val &= ~psu_mux_mask;
            //modify psu_mux_sel bits (bit 1 and 2)
            reg_val |= mux_sel_val;
            //write reg value
            s8901_54xc_cpld_write_internal(cpld_node->client, reg, reg_val);
...
EXPORT_SYMBOL(s8901_54xc_cpld_psu_mux_sel);
...
static struct i2c_driver cpld_driver = {
    ...
    .driver = {
        .name = "x86_64_ufispace_s8901_54xc_cpld",
    },
    .probe = cpld_probe,
    .remove = cpld_remove,
    .id_table = cpld_id,

...
uint8_t ipmi_check_psu_present(uint8_t psu_slot)
{
	...
	if (rv) {
		...
		slot_mask = (psu_slot == 1) ? 0x02 : 0x01; // for IX8C, PSU order is different from other QCT projects
...
static ssize_t show_fanpresent(struct device *dev, struct device_attribute *devattr, char *buf)
{
	...
	fan_idx = 5 - fan_idx; // for IX8C, FAN order is different from other QCT projects

...
get_BMC_response_fail:
  ...
  if (ipmi_wait_queue == 0)
  {
    ...
    if (rv == 0)
    {
      complete(comm_data->rx_read_complete);
    }
  }
}
static struct ipmi_user_hndl ipmi_hndlrs = { .ipmi_recv_hndl = ipmi_msg_handler, };

int32_t ipmi_request_wait_for_response(struct kernel_ipmi_msg msg,
                                       struct ipmi_comm_data *comm_data)
{
  int32_t rv = 0;
  ...
  ipmi_request_settime(ipmi_mh_user, &ipmi_address, comm_data->tx_id, &msg,
                       comm_data, 0, 0, 0);

  escape_time = wait_for_completion_timeout(comm_data->rx_read_complete,
                                            IPMI_TIMEOUT);
...
int32_t ipmi_send_system_cmd(uint8_t *msg_tx_data, int32_t msg_tx_len,
                             void *msg_rx_data, int32_t msg_rx_len)
{
  int32_t i = 0;
  int32_t rv = 0;
...
int32_t ipmi_sdr_get_header(struct ipmi_sdr_iterator *itr,
...
int32_t ipmi_sdr_get_record(struct sdr_get_rs *header,
                            struct ipmi_sdr_iterator  *itr, uint8_t *ret_data)
{
  int32_t rv = 0, len = 0;
...
void ipmi_sdr_set_sensor_threshold(uint8_t idx,
                                   struct sdr_record_full_sensor *sensor)
{
  ...
  {
    g_sensor_data[idx].capability |= SENSOR_GET_CAP_LNC;
  }
  if (sensor->mask.type.threshold.read.lcr)
  {
    g_sensor_data[idx].capability |= SENSOR_GET_CAP_LCR;
  }
  if (sensor->mask.type.threshold.read.lnr)
  {
    g_sensor_data[idx].capability |= SENSOR_GET_CAP_LNR;
  }
  ...
  {
    g_sensor_data[idx].capability |= SENSOR_GET_CAP_UNC;
  }
  if (sensor->mask.type.threshold.read.ucr)
  {
    g_sensor_data[idx].capability |= SENSOR_GET_CAP_UCR;
  }
  if (sensor->mask.type.threshold.read.unr)
  {
    g_sensor_data[idx].capability |= SENSOR_GET_CAP_UNR;
  }
...
void ipmi_sdr_set_sensor_factor(uint8_t idx,
                                struct sdr_record_full_sensor *sensor)
{
  char *loc = NULL;
  ...
  memset(g_sensor_data[idx].attrinfo.attr_type_str, 0x00,
         SENSOR_ATTR_NAME_LENGTH);
...
#if enable_debug_msg
  {
    ...
    printk("m[%d], b[%d], k1[%d], k2[%d]\n", g_sensor_data[idx].recordinfo.record_m,
           g_sensor_data[idx].recordinfo.record_b
           , g_sensor_data[idx].recordinfo.record_k1,
           g_sensor_data[idx].recordinfo.record_k2);

    printk("sensor [%s] type[%d], analog[%d], linearization[%d]\n",
           g_sensor_data[idx].sensor_idstring, g_sensor_data[idx].sensor_type
           , g_sensor_data[idx].recordinfo.record_analog,
           g_sensor_data[idx].recordinfo.record_linearization);
...
int32_t sdr_convert_sensor_reading(uint8_t idx, uint8_t val,
                                   int32_t *point_result)
{
...
int32_t ipmi_sdr_parsing_value(int32_t idx, uint8_t input_value,
                               int8_t *ret_str)
{
  ...
  {
    snprintf(ret_str, temp_len + 1, "%s",
             temp_str);  /* +1 for snprintf reserve space'\0' */
  }
  ...
  /* float part */
  if ((point_result == 0) || (temp_len < 0))
  {
    strcat(ret_str, "0");
  }
  else
  {
    strcat(ret_str, temp_str + temp_len);
  }

  /* EOL part */
  strcat(ret_str, "\n\0");

  return (temp_len + 1 + point_result +
          2); /*integer + point + float + EOL + \0 */
}
...
uint8_t ipmi_check_psu_present(uint8_t psu_slot)
{
  ...
  /*uint8_t msg_data[] = { 0x06, 0x52, 0x1B, 0x4C, 0x01, 0x00 }; //netfn = IPMI_NETFN_APP; cmd = GET_PSU_READING; */
...
int32_t ipmi_get_psu_info(uint8_t idx, uint8_t cmd, uint8_t *retbuf)
{
  ...
  uint8_t returnData[128] = { 0 };
  uint8_t msg_data[] = { 0x36, 0xBB, 0x4C, 0x1C, 0x00, cmd, 0x00 }; /*netfn = 0x36; cmd = 0xBB; */

  if (strstr(g_sensor_data[idx].sensor_idstring, "PSU1"))
  {
    psu_slot = 1;
  }
  else
...
void ipmi_fan_control(uint8_t cmd_data1, uint8_t cmd_data2, uint8_t *retbuf)
{
  ...
  if (cmd_data1)
  {
    rv = ipmi_send_system_cmd(msg_data, sizeof(msg_data), NULL, 0);
  }
  else
  {
    rv = ipmi_send_system_cmd(msg_data, sizeof(msg_data), returnData, 1);
...
static ssize_t show_input(struct device *dev, struct device_attribute *devattr,
                          char *buf)
{
  struct sensor_device_attribute *attr = to_sensor_dev_attr(devattr);
  int32_t rv = 0;
...
static ssize_t show_lnr(struct device *dev, struct device_attribute *devattr,
                        char *buf)
{
  struct sensor_device_attribute *attr = to_sensor_dev_attr(devattr);
  return ipmi_sdr_parsing_value(attr->index + DEBUGUSE_SHIFT,
                                g_sensor_data[attr->index + DEBUGUSE_SHIFT].lowerinfo.lnr, buf);
}

static ssize_t show_lcr(struct device *dev, struct device_attribute *devattr,
                        char *buf)
{
  struct sensor_device_attribute *attr = to_sensor_dev_attr(devattr);
  return ipmi_sdr_parsing_value(attr->index + DEBUGUSE_SHIFT,
                                g_sensor_data[attr->index + DEBUGUSE_SHIFT].lowerinfo.lcr, buf);
}

static ssize_t show_lnc(struct device *dev, struct device_attribute *devattr,
                        char *buf)
{
  struct sensor_device_attribute *attr = to_sensor_dev_attr(devattr);
  return ipmi_sdr_parsing_value(attr->index + DEBUGUSE_SHIFT,
                                g_sensor_data[attr->index + DEBUGUSE_SHIFT].lowerinfo.lnc, buf);
}

static ssize_t show_unr(struct device *dev, struct device_attribute *devattr,
                        char *buf)
{
  struct sensor_device_attribute *attr = to_sensor_dev_attr(devattr);
  return ipmi_sdr_parsing_value(attr->index + DEBUGUSE_SHIFT,
                                g_sensor_data[attr->index + DEBUGUSE_SHIFT].upperinfo.unr, buf);
}

static ssize_t show_ucr(struct device *dev, struct device_attribute *devattr,
                        char *buf)
{
  struct sensor_device_attribute *attr = to_sensor_dev_attr(devattr);
  return ipmi_sdr_parsing_value(attr->index + DEBUGUSE_SHIFT,
                                g_sensor_data[attr->index + DEBUGUSE_SHIFT].upperinfo.ucr, buf);
}

static ssize_t show_unc(struct device *dev, struct device_attribute *devattr,
                        char *buf)
{
  struct sensor_device_attribute *attr = to_sensor_dev_attr(devattr);
  return ipmi_sdr_parsing_value(attr->index + DEBUGUSE_SHIFT,
                                g_sensor_data[attr->index + DEBUGUSE_SHIFT].upperinfo.unc, buf);
}

static ssize_t show_model(struct device *dev, struct device_attribute *devattr,
                          char *buf)
{
  struct sensor_device_attribute *attr = to_sensor_dev_attr(devattr);
...
static ssize_t show_mfrid(struct device *dev, struct device_attribute *devattr,
                          char *buf)
{
  struct sensor_device_attribute *attr = to_sensor_dev_attr(devattr);
                         ...
                         const char *buf, size_t count)
{
  ...
  if (g_fan_control_mode == 1)
  {
    ipmi_fan_control(0x01, store_input, returnData);
  }

  return count;
}

static ssize_t show_controlmode(struct device *dev,
                                struct device_attribute *devattr, char *buf)
{
  return sprintf(buf, "%d\n", g_fan_control_mode);
}

static ssize_t store_controlmode(struct device *dev,
                                 struct device_attribute *devattr, const char *buf, size_t count)
{
  ...
  if (g_fan_control_mode == 3)
  {
    ipmi_fan_control(0x7f, 0xff, returnData);
  }
...
static ssize_t show_fanpresent(struct device *dev,
                               struct device_attribute *devattr, char *buf)
{
  int32_t rv = 0;
...
static ssize_t show_psupresent(struct device *dev,
                               struct device_attribute *devattr, char *buf)
{
  int32_t psu_idx = 0;
...
static ssize_t(*const attr_show_func_ptr[SENSOR_ATTR_MAX])(struct device *dev,
                                                           struct device_attribute *devattr, char *buf) =
{
...
static int32_t create_sensor_attrs(int32_t attr_no)
{
  ...
  for (i = 0; i < SENSOR_ATTR_MAX; i++)
  {
    if ((g_sensor_data[attr_no].capability >> i) & 0x01)
    {
      ...
      if (attrdata->sd_attrs[j].dev_attr.store)
      {
        attrdata->sd_attrs[j].dev_attr.attr.mode |= S_IWUSR;
      }
...
static int32_t remove_sensor_attrs(void)
{
  ...
  for (i = 0; i < data->total_sensor_id; i++)
  {
    ...
    {
      sysfs_remove_group(&data->ipmi_hwmon_dev->kobj,
                         &g_sensor_data[i].attrinfo.attr_group);
    }
  }
  return 0;
}
...
int32_t ipmi_init_sdr_sensors_data(void)
{
  ...
  g_sensor_data = kzalloc(itr->total * sizeof(struct ipmi_sensor_data),
                          GFP_KERNEL);
  ...
  for (sdr_idx = 0; sdr_idx < itr->total; sdr_idx++)
  {
    ...
    switch (header->type)
    {
      case SDR_RECORD_TYPE_FULL_SENSOR:
        ...
        g_sensor_data[sdr_idx].capability =
          SENSOR_GET_CAP_LABEL /*| SENSOR_GET_CAP_ALARM */ | SENSOR_GET_CAP_INPUT;
        ...
        if (sdr_idx >= DEBUGUSE_SHIFT)
        {
          ...
          if (err)
          {
            ...
            printk("[err : %d]sysfs_create_group fail in [%d] %s\n", err, sdr_idx,
                   g_sensor_data[sdr_idx].sensor_idstring);

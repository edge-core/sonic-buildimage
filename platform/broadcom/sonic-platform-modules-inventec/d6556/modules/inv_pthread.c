...
int status_led_psu_control(void);
ssize_t status_fan_led(const char *freq);
ssize_t status_psu_led(const char *freq);
...
int sysfs_detect_hwmon_index(void)
{
    ...
    char hwmon_dev_path[MAX_PATH_SIZE];
    int hwid = 0;
	...
	sprintf(hwmon_dev_path, "/sys/class/hwmon/hwmon%d/device/name", hwid);
	inventec_show_attr(hwmon_buf, hwmon_dev_path);
...
#define PSU_STATE_UNPOWER	("2 : unnormal")
...
static char psu_0_dev_path_state[MAX_PATH_SIZE];
static char psu_1_dev_path_state[MAX_PATH_SIZE];
...
int status_led_psu_control(void)
{
    int retVal = 0; //no issue

    char buf[MAX_ACC_SIZE];

    int normal_psu_cnt = 0; //normal

    int fault_psu_cnt = 0;

    //read fan status one by one
    //read psu 0 status
    if (inventec_show_attr(&buf[0], psu_0_dev_path_state) > 0)
    {
        if(buf[0] == '0') //normal
        {
            normal_psu_cnt = normal_psu_cnt + 1;
        }
        else if (buf[0] == '4') // psu fault
        {
            fault_psu_cnt = fault_psu_cnt + 1;
        }

        //Check 2nd psu
        if(inventec_show_attr(&buf[0], psu_1_dev_path_state) > 0)
        {
            if(buf[0] == '0') //normal
            {
                normal_psu_cnt = normal_psu_cnt + 1;
    ...
    else
    {
        if( fault_psu_cnt > 0)
        {
            status_psu_led("0x01");  //Red lit
            retVal = 1;
        }
        else if (normal_psu_cnt == 1)
        {
            status_psu_led("0x20");  // Green blink
            retVal = 1;
        }
        else if (normal_psu_cnt == 2)
        {
            status_psu_led("0x10"); // Green Lit
            retVal = 0;
        }
    }
...
#define STATUS_LED_FAN_PATH     "/sys/class/hwmon/hwmon%d/device/fan_led"
#define STATUS_LED_PSU_PATH     "/sys/class/hwmon/hwmon%d/device/psu_led"
...
static char status_led_fan_path[MAX_PATH_SIZE];
static char status_led_psu_path[MAX_PATH_SIZE];
...
//status led: FAN & PSU
ssize_t
status_fan_led(const char *freq)
{
    ssize_t ret;
    ret = inventec_store_attr(freq, strlen(freq), &status_led_fan_path[0]);
    if (ret < 0) {
        return ret;
    }
...
ssize_t
status_psu_led(const char *freq)
{
    ssize_t ret;
    ret = inventec_store_attr(freq, strlen(freq), &status_led_psu_path[0]);
    if (ret < 0) {
        return ret;
    }
...
{
    ssize_t ret = inventec_store_attr("253", 2, "/sys/class/gpio/export");
    if (ret < 0) {
...
{
    bool fan_issue = false;
    bool psu_issue = false;
    bool psu_number_issue = false;
	...
	//switch_temp_update();
        ...
        if (status_led_psu_control() > 0)
        {
            psu_number_issue = true;
        }

        if(fan_issue || psu_issue || psu_number_issue)
        {
            continue;
        }

...
enum cpld_type {
    ...
    as9736_64d_cpld_pdb,
    as9736_64d_cpld_scm,
    as9736_64d_cpld_udb  /*No attribute but add i2c addr to the list.*/
};
...
static const struct i2c_device_id as9736_64d_cpld_id[] = {
    ...
    { "as9736_64d_cpld_udb", as9736_64d_cpld_udb},
    { }
};
...
static int as9736_64d_cpld_write_internal(struct i2c_client *client, u8 reg, u8 value);
...
/* CPU-CPLD, CPLD1, CPLD2 Attributes*/
static SENSOR_DEVICE_ATTR(psu1_present, S_IRUGO, show_status, NULL, PSU_LEFT_PRESENT);
static SENSOR_DEVICE_ATTR(psu2_present, S_IRUGO, show_status, NULL, PSU_RIGHT_PRESENT);
static SENSOR_DEVICE_ATTR(psu1_power_good, S_IRUGO, show_status, NULL, PSU_LEFT_POWER_GOOD);
static SENSOR_DEVICE_ATTR(psu2_power_good, S_IRUGO, show_status, NULL, PSU_RIGHT_POWER_GOOD);
...
static struct attribute *as9736_64d_cpld_cpu_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_psu1_present.dev_attr.attr, /*left*/
    &sensor_dev_attr_psu2_present.dev_attr.attr, /*right*/
    &sensor_dev_attr_psu1_power_good.dev_attr.attr, /*left*/
    &sensor_dev_attr_psu2_power_good.dev_attr.attr, /*right*/
    NULL
};
...
static const struct attribute_group as9736_64d_cpld_udb_group = {
    .attrs = as9736_64d_cpld_udb_attributes,
};
             ...
             char *buf)
{
    ...
    status = as9736_64d_cpld_read_internal(client, reg);
             ...
             char *buf)
{
    ...
    switch (attr->index) {
    case PSU_RIGHT_PRESENT:
    case PSU_LEFT_PRESENT:
        reg = 0x14;
        mask = 0x1 << attr->index;
        revert = 1;
        break;
    case PSU_LEFT_POWER_GOOD:
    case PSU_RIGHT_POWER_GOOD:
        reg = 0x90;
        mask = 0x1 << (attr->index - PSU_LEFT_POWER_GOOD);
        break;
            ...
            const char *buf, size_t count)
{
    ...
    status = as9736_64d_cpld_write_internal(client, reg, status);
    if (unlikely(status < 0)) {
        goto exit;
    }
            ...
            const char *buf, size_t count)
{
    ...
    status = as9736_64d_cpld_write_internal(client, reg, val);
    if (unlikely(status < 0)) {
        goto exit;
    }
    mutex_unlock(&data->update_lock);
             ...
             const struct i2c_device_id *id)
{
    ...
    case as9736_64d_cpld_scm:
        ...
        break;
    case as9736_64d_cpld_udb:
        group = &as9736_64d_cpld_udb_group;
        break;
    default:
        break;
    }
...
int as9736_64d_cpld_read(unsigned short cpld_addr, u8 reg)
{
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = as9736_64d_cpld_read_internal(cpld_node->client, reg);
            break;
        }
    }
...
int as9736_64d_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = as9736_64d_cpld_write_internal(cpld_node->client, reg, value);
            break;
        }
    }

...
struct as9817_64_fan_data *data = NULL;

static struct platform_driver as9817_64_fan_driver = {
...
enum as9817_64_fan_sysfs_attrs {
    FAN_ATTR(1),
    FAN_ATTR(2),
    FAN_ATTR(3),
    FAN_ATTR(4),
...
static struct attribute *as9817_64_fan_attrs[] = {
    /* fan attributes */
    DECLARE_FAN_ATTR(1),
    DECLARE_FAN_ATTR(2),
    DECLARE_FAN_ATTR(3),
...
ATTRIBUTE_GROUPS(as9817_64_fan);
...
static void ipmi_msg_handler(struct ipmi_recv_msg *msg, void *user_msg_data)
{
    ...
    if (msg->msgid != ipmi->tx_msgid) {
        dev_notice(&data->pdev->dev, "Mismatch between received msgid "
...
static struct as9817_64_fan_data *as9817_64_fan_update_device(void)
{
    int status = 0;
                            ...
                            char *buf)
{
    ...
    data = as9817_64_fan_update_device();
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
...
static struct as9817_64_fan_data *as9817_64_fan_update_cpld_ver(void)
{
    int status = 0;
                                ...
                                char *buf)
{
    ...
    data = as9817_64_fan_update_cpld_ver();
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
...
static int __init as9817_64_fan_init(void)
{
    ...
    ret = platform_driver_register(&as9817_64_fan_driver);
    if (ret < 0)
        goto dri_reg_err;
...
dev_reg_err:
    platform_driver_unregister(&as9817_64_fan_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as9817_64_fan_exit(void)
{
    if (data) {
...
MODULE_DESCRIPTION("as9817_64_fan driver");
MODULE_LICENSE("GPL");

module_init(as9817_64_fan_init);
module_exit(as9817_64_fan_exit);

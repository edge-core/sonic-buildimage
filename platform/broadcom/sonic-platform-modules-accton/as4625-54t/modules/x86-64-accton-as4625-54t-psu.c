...
static int as4625_54t_psu_read_byte(struct i2c_client *client, u8 command, u8 *data)
{
	int status = 0;
	int retry_count = 5;
...
static int as4625_54t_psu_read_bytes(struct i2c_client *client, u8 command, u8 *data,
			  int data_len)
{
	int ret = 0;
...
static struct as4625_54t_psu_data *as4625_54t_psu_update_device(struct device *dev)
{
	...
	if (time_after(jiffies, data->last_updated + HZ + HZ / 2)
		...
		if (IS_POWER_GOOD(data->index, data->status)) {
			/* Read model name */
			status = as4625_54t_psu_read_bytes(client, MODEL_NAME_REG_OFFSET, data->model_name,
			...
			/* Read serial number */
			status = as4625_54t_psu_read_bytes(client, SERIAL_NUM_REG_OFFSET, data->serial,

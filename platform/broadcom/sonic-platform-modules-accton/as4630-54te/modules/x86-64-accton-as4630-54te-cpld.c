...
/*fan sysfs*/
static struct as4630_54te_cpld_data *as4630_54te_fan_update_device(struct device *dev);
			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	status = as4630_54te_cpld_write_internal(client, addr, val);
	if (unlikely(status < 0)) {
		goto exit;
	}
	mutex_unlock(&data->update_lock);
...
static void as4630_54te_cpld_add_client(struct i2c_client *client)
{
...
static void as4630_54te_cpld_remove_client(struct i2c_client *client)
{
...
static struct as4630_54te_cpld_data *as4630_54te_fan_update_device(struct device *dev)
{
    ...
    if (time_after(jiffies, data->last_updated + HZ + HZ / 2) ||
        ...
        dev_dbg(&client->dev, "Starting as4630_54te_fan update\n");
        data->valid = 0;

        /* Update fan data
         */
        for (i = 0; i < ARRAY_SIZE(data->reg_fan_val); i++) {
            int status = as4630_54te_cpld_read_internal(client, fan_reg[i]);
            if (status < 0) {
                data->valid = 0;
                mutex_unlock(&data->update_lock);
...
//	int status;	
	...
	data = kzalloc(sizeof(struct as4630_54te_cpld_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto exit;
	}
...
int as4630_54te_cpld_read(unsigned short cpld_addr, u8 reg)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EPERM;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = as4630_54te_cpld_read_internal(cpld_node->client, reg);
    		break;
        }
    }
...
EXPORT_SYMBOL(as4630_54te_cpld_read);

int as4630_54te_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EIO;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = as4630_54te_cpld_write_internal(cpld_node->client, reg, value);
            break;
        }
    }
...
EXPORT_SYMBOL(as4630_54te_cpld_write);

static struct i2c_driver as4630_54te_cpld_driver = {
	.driver		= {
...
static int __init as4630_54te_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&as4630_54te_cpld_driver);
}

static void __exit as4630_54te_cpld_exit(void)
{
    i2c_del_driver(&as4630_54te_cpld_driver);
}
...
module_init(as4630_54te_cpld_init);
module_exit(as4630_54te_cpld_exit);

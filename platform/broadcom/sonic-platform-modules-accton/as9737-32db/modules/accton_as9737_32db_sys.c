    ...
    for (retry = 0; retry <= IPMI_ERR_RETRY_TIMES; retry++) {
        ...
        if (unlikely(status != 0)) {
            dev_err(&data->pdev->dev,
                    "ipmi_send_message_%d err status(%d)[%s] raw_cmd=[%s]\r\n",
        ...
        if (unlikely(ipmi->rx_result != 0)) {
            dev_err(&data->pdev->dev,
                    "ipmi_send_message_%d err rx_result(%d)[%s] raw_cmd=[%s]\r\n",
...
static struct as9737_32db_sys_data *as9737_32db_sys_update_fpga_ver(void)
{
    int status = 0;
                                ...
                                struct device_attribute *da, char *buf)
{
    ...
    data = as9737_32db_sys_update_fpga_ver();
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }

...
#ifdef HAS_CHANNEL_REG
#define OCI2C_CHANNEL        5 /* Assign port num; Only for AS9737-32DB FPGA */
#endif
...
MODULE_PARM_DESC(timeout, "Timeout for ocores_poll_wait, measured in milliseconds.");
...
static void ocores_process(struct ocores_i2c *i2c, u8 stat)
{
    ...
    if ((i2c->state == STATE_START) || (i2c->state == STATE_WRITE)) {
        ...
        if (stat & OCI2C_STAT_NACK) {
            i2c->state = STATE_ERROR;
            if (debug) {
                dev_warn(dev, "I2C %s, no ACK from slave 0x%02x (%s)", i2c->adap.name, msg->addr, cmdline ? cmdline : "");
            }
            oc_setreg(i2c, OCI2C_CMD, OCI2C_CMD_STOP);
            goto out;
        }
                            ...
                            bool polling)
{
    ...
    /* Only for AS9737-32DB FPGA */
    oc_setreg(i2c, OCI2C_CHANNEL, i2c->channel);
#endif
...
#ifdef HAS_CHANNEL_REG
    dev_info(dev, "OCI2C_CHANNEL=0x%02x OCI2C_PRELOW=0x%02x OCI2C_PREHIGH=0x%02x\n",
                  i2c->channel, prescale & 0xff, prescale >> 8);
#else
                                            ...
                                            const struct resource *res)
{
    ...
    if (res->name)
        pretty_name = devm_kasprintf(dev, GFP_KERNEL, "%s %s", dev_name(dev), res->name);
    else
    ...
    /***********************************************
     * Cannot use devm_request_mem_region in here. *
     ***********************************************/
...
static int ocores_i2c_probe(struct platform_device *pdev)
{
    ...
    if (res) {
#ifdef HAS_CHANNEL_REG
        i2c->base = devm_ioremap_shared_resource(&pdev->dev, res);
#else
        i2c->base = devm_ioremap_resource(&pdev->dev, res);
#endif
        dev_info(&pdev->dev, "Resouce Memory: start:0x%llx, end:0x%llx", res->start, res->end);
        if (IS_ERR(i2c->base))
...
#ifdef HAS_CHANNEL_REG
    res = platform_get_resource_byname(pdev, IORESOURCE_REG, "channel");
    if (res) {
        i2c->channel = res->start;
        dev_info(&pdev->dev, "Resouce Channel: 0x%llx", res->start);
    } else {
        dev_err(&pdev->dev, "No Channel resource for port number identification.\n");
        return -EINVAL;
    }
#endif

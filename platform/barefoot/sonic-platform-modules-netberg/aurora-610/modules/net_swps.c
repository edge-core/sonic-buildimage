...
get_layout_info(void){
    ...
    switch (platform_p->id) {
        ...
        case PLATFORM_TYPE_AURORA_610:
            ...
            ioexp_layout  = aurora_610_ioexp_layout;
            port_layout   = aurora_610_port_layout;
            ioexp_total   = ARRAY_SIZE(aurora_610_ioexp_layout);
            port_total    = ARRAY_SIZE(aurora_610_port_layout);
            break;
#endif
        default:
...
swp_module_init(void){
    ...
    SWPS_INFO("Netberg switch-port module V.%s initial success.\n", SWP_VERSION);
    return 0;
...
err_init_ioexp:
    ...
    unregister_chrdev_region(MKDEV(port_major, 0), port_total);
err_init_out:
    SWPS_ERR("Netberg switch-port module V.%s initial failure.\n", SWP_VERSION);
    return -1;
}
...
swp_module_exit(void){
    ...
    unregister_chrdev_region(MKDEV(ctl_major, 0), 1);
    unregister_chrdev_region(MKDEV(port_major, 0), port_total);
    SWPS_INFO("Remove Netberg switch-port module success.\n");
}

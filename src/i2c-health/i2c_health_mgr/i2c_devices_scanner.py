...
class I2CDevicesScanner:
    ...
    def _drain_stale_events(self):
        """
        Drain any leftover subscription events from restore_state_db_records()'s  table.set()
        ...
        while True:
            ...
            for device_tbl in self.asic_context.keys():
                while True:
                    ...
                    #self.logger.log_notice("Draining stale events table name: {}, key: {}, op: {}, field value pair: {}".format(device_tbl.table_name, key, op, fvp))
                    if not key:
                        break

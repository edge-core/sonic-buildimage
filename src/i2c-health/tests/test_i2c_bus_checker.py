...
def test_is_bus_lock_no(mock_subprocess_run, mock_platform_api, mock_logger):
    mock_platform_api.get_i2c_representative_dev_list.return_value = [
        {"name": "Device1", "bus": "12", "device_addr": "0x48", "register_addr": "0x00"},
        {"name": "Device2", "bus": "14", "device_addr": "0x1F", "register_addr": "0x00"}
    ]
    mock_subprocess_run.return_value.returncode = 0
    i2c_checker = I2CBusChecker(mock_logger, mock_platform_api)

    result = i2c_checker.is_bus_lock()
    assert result is False

@patch('i2c_bus_checker.subprocess.run')
def test_is_bus_lock_yes(mock_subprocess_run, mock_platform_api, mock_logger):
    mock_platform_api.get_i2c_representative_dev_list.return_value = [
        {"name": "Device1", "bus": "12", "device_addr": "0x48", "register_addr": "0x00"},
        {"name": "Device2", "bus": "14", "device_addr": "0x1F", "register_addr": "0x00"}
    ]
...
def test_i2c_representative_list_get(mock_platform_api, mock_logger):
    ...
    mock_platform_api.get_i2c_representative_dev_list.return_value = original_dev_list

    i2c_checker = I2CBusChecker(mock_logger, mock_platform_api)
    result = i2c_checker.i2c_representative_list_get()

    assert result == original_dev_list

...
void initialize_counter(std::string counterVlan) {
    ...
    m_stateDbRedisClient.hset(counterVlan, "Malformed", toString(counters[DHCPv6_MESSAGE_TYPE_MALFORMED]));
}
        ...
        case DHCPv6_MESSAGE_TYPE_INFORMATION_REQUEST:
        {
            while (option_position - message_buffer < len) {
                ...
                if (ntohs(option->option_code) > DHCPv6_OPTION_LIMIT) {
                    ...
                    update_counter(counterVlan.append(config->interface), DHCPv6_MESSAGE_TYPE_MALFORMED);

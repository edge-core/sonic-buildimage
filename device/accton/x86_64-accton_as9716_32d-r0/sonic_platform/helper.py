...
class APIHelper():
    ...
    def __init__(self):
        ...
        if not os.path.isfile(SONIC_BOOT):
            # Creates a new file for record bootup and remove eeprom cache file(if exist)
            with open(SONIC_BOOT, 'w') as fp:
                pass
    ...
    def run_command_sync(self, cmd, shell=False):
        ...
        if not shell and not isinstance(cmd, (list, tuple)):
            raise ValueError("cmd must be a list/tuple when shell=False")
        ...
        try:
            ...
            raise RuntimeError(
                "Command failed '{}', returncode {}: stdout='{}', stderr='{}'".format(

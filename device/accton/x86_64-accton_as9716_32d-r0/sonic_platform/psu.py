...
class Psu(PsuBase):
    ...
    def get_i2c_list(self):
        """
        Retrieves I2C device information for the PSU and its subcomponents.

        Extracts bus and device addresses from both HWMON and CPLD paths, and appends 
        I2C data from related fans and thermal components to the list.

        Returns:
            list: A list with I2C device details for the PSU and subcomponents.
        """
        i2c_list = []
        if self.index == 0:
            region = '77-1-72-1'
        ...
        path_parts = self.hwmon_path.rsplit('/', 2)[1].split('-')
        ...
        path_parts = self.cpld_path.rsplit('/', 2)[1].split('-')
        bus = int(path_parts[0])
        device_addr = int(path_parts[1], 16)
        i2c_list.append({
            'name': self.get_name(),
    ...
    def set_faulty_device(self, faulty):
        cpld_faulty_device_path="{}{}".format(self.cpld_path, 'psu_faulty_device')
        hwmon_faulty_device_path="{}{}".format(self.hwmon_path, 'psu_faulty_device')

        val = '1' if faulty else '0'

        all_success = True
        for path in [cpld_faulty_device_path, hwmon_faulty_device_path]:
            if not self._api_helper.write_txt_file(path, val):
                all_success = False
